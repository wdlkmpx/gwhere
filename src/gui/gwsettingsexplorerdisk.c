...
gint gw_plugin_settings_explorer_disk_pane_create ( GtkWindow *settings, GtkContainer *parent, GtkWidget **pane)
{
	...
	if ( settings != NULL && parent != NULL )
	{
		...
		*pane = hbox;
		...
		g_signal_connect (G_OBJECT ( hbox), "destroy", (GCallback)gw_plugin_settings_explorer_disk_destroy, NULL);
		g_signal_connect (G_OBJECT ( btn_hidden), "clicked", (GCallback)gw_plugin_settings_explorer_disk_btn_hide_click, hbox);
		g_signal_connect (G_OBJECT ( btn_show), "clicked", (GCallback)gw_plugin_settings_explorer_disk_btn_show_click, hbox);
...
gint gw_plugin_settings_explorer_disk_pane_load ( GtkWidget *pane)
{
	...
	if ( pane != NULL )
	{
		list_visible = GTK_CLIST ( g_object_get_data (G_OBJECT ( pane), GW_PLUGIN_SETTINGS_EXPLORER_DISK_LIST_VISIBLE));
		list_hidden = GTK_CLIST ( g_object_get_data (G_OBJECT ( pane), GW_PLUGIN_SETTINGS_EXPLORER_DISK_LIST_HIDDEN));

		if ( (list_visible != NULL) && (list_hidden != NULL) )
		{
			if ( (fields = gw_plugin_settings_explorer_disk_get_all_fields ( )) != NULL )
			{
			g_object_set_data_full (G_OBJECT ( pane), GW_PLUGIN_SETTINGS_EXPLORER_DISK_FIELDS_TABLE, fields, (GDestroyNotify) gw_settings_explorer_field_freev);
...
gint gw_plugin_settings_explorer_disk_pane_apply ( GtkWidget *pane)
{
	...
	if ( pane != NULL )
	{
		list_visible = GTK_CLIST ( g_object_get_data (G_OBJECT ( pane), GW_PLUGIN_SETTINGS_EXPLORER_DISK_LIST_VISIBLE));
		list_hidden = GTK_CLIST ( g_object_get_data (G_OBJECT ( pane), GW_PLUGIN_SETTINGS_EXPLORER_DISK_LIST_HIDDEN));
		fields = (GWSettingsExplorerField**)g_object_get_data (G_OBJECT ( pane), GW_PLUGIN_SETTINGS_EXPLORER_DISK_FIELDS_TABLE);
