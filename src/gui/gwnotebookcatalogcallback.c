		...
		if ( GTK_CTREE_ROW ( node)->parent == NULL ) {
			...
			if ( (disks = plugin->gw_db_catalog_get_db_disks ( context)) != NULL) {
				for ( i = 0; disks[i] != NULL; i++) {
					...
					tempstr = g_strdup_printf ( "%d", gw_db_disk_get_num ( disks[i]));
					g_strdup_to_gtk_text ( tempstr, info[GW_VALUE_APP_EXPLORER_DISK_COLUMN_NBARCHIVE_POSITION_DEFAULT]);
					g_free ( tempstr);
					...
					tempstr = gw_ui64_byte_to_str_format ( gw_db_disk_get_capacity ( disks[i]));
					g_strdup_to_gtk_text ( tempstr, info[GW_VALUE_APP_EXPLORER_DISK_COLUMN_CAPACITY_POSITION_DEFAULT]);
					g_free ( tempstr);
					tempstr = gw_ui64_byte_to_str_format ( gw_db_disk_get_free ( disks[i]));
					g_strdup_to_gtk_text ( tempstr, info[GW_VALUE_APP_EXPLORER_DISK_COLUMN_FREE_POSITION_DEFAULT]);
					g_free ( tempstr);
			...
			if ( files != NULL ) {
				for ( i = 0; files[i] != NULL; i++) {
					...
					if ( strcmp ( gw_db_file_get_name ( files[i]), ".") != 0) {
						...
						tempstr = gw_ui64_byte_to_str_format ( gw_db_file_get_size ( files[i]));
						g_strdup_to_gtk_text ( tempstr, info[GW_VALUE_APP_EXPLORER_FILE_COLUMN_SIZE_POSITION_DEFAULT]);
						g_free ( tempstr);
						tempstr = g_strdup_printf ( "%ld", gw_db_file_get_inode ( files[i]));
						g_strdup_to_gtk_text ( tempstr, info[GW_VALUE_APP_EXPLORER_FILE_COLUMN_INODE_POSITION_DEFAULT]);
						g_free ( tempstr);
