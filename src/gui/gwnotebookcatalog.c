...
GtkCList * gw_notebook_catalog_get_clist_explorer ( GtkWindow *w) {
	...
	if ( w != NULL ) {
		clist_explorer = GTK_CLIST ( g_object_get_data (G_OBJECT ( w), GW_REF_CLIST_EXPLORER));
	}
...
gint gw_notebook_catalog_get_hpan_position ( GtkWindow *w) {
	...
	if ( w != NULL ) {
		if ( (hpan = GTK_PANED ( g_object_get_data (G_OBJECT ( w), GW_REF_NOTEBOOK_CATALOG_PANE))) != NULL) {
